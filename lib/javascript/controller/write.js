...
export const modalCarendarReciver = (go_date, come_date) => {
  go_day_el.innerText = `${go_date.month}월 ${go_date.day}일`;
  come_day_el.innerText = `${come_date.month}월 ${come_date.day}일`;

  go_day.value = `${go_date.year}-${go_date.month}-${go_date.day}`
  come_day.value = `${come_date.year}-${come_date.month}-${come_date.day}`
}
...
writeForm.addEventListener("submit", async function(event) {
    ...
    let writeJson = {
      ...
      'logingInfo': {
        ...
        'lodgingPrice': lodging_price.value.trim()
      },
      'taxPrice' : tax_price.value.trim(),
