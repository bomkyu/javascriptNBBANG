...
async function friendAdd() {
    console.log('friendAdd 함수 바로 밑입니다.');
...
const friendList = async () => {
    ...
    //친구목록 가져오기
    const friendRequests = friend_request_data.filter(param => param.requestFriend === session && param.status === 'accept');
    const friendIds = friendRequests.map(param => param.sendFriend);
    const friendList = user_data.filter(user => friendIds.includes(user.id));
