...
async function friendAdd() {
    ...
    const friend_data = await Select('FriendRequest');
    ...
    function handleInputEvent() {
      ...
      const joinedData = user_data
      .filter(user => user.id !== session)
      .map(user => {
        const userFriendRequests = friend_data.filter(request => {
          return (
            request.sendFriend === user.id || request.requestFriend === user.id
          );
        });
...
const requestFriend = async (v, sort) => {
    ...
    switch (sort) {
        case 'wait':
            ...
            await Insert('FriendRequest', userJson);
            friendAdd();
            break;
        case 'accept':
            //console.log(v);
            ...
            await Update('FriendRequest','sq',v,'status','accept');
            friendList();
            break;
