...
const createHtml = async () => {
    ...
    try {
        ...
        if(trip_filter.length == 0 || trip_filter == null){
            ...
            contain_el.innerHTML = ''
            let html = '';
            const {manager, goDay, comeDay, placeSearch, friends, taxPrice, TaxImg, placePrice} = trip_filter[0];
            const { name, address, location : {lat, lng} } = placeSearch;
            const king_filter = users.filter(param => param.id == trip_filter[0].manager);
            trip_filter.forEach(param => {
                ...
                html += `
                    ...
                    friends.forEach(frinedss => {
                        console.log('ffffffffffffffffffff', trip_filter)
                    let { imgUrl,userName} = frinedss;
                ...
                html += `
                            ...
                            <dl class="location-info">
                                <dt>가격 : </dt>
                                <dd>${numberWithCommas(Number(placePrice))}원</dd>
                            </dl>
                        `
                        let total_pay = Number(placePrice) + Number(taxPrice);
                ...
                html += `
                            ...
                            <h2 class="totl-price">인당금액 : ${numberWithCommas(Number(total_pay / (friends.length + 1)))}원</h2>
                        `
