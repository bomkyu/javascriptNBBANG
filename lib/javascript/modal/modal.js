    ...
    case 'calendar':
      ...
      calendarList.forEach((li) => {
        li.addEventListener('click', (e) => {
          if (
            li.classList.contains('previous-day') ||
            li.classList.contains('previous-month') ||
            li.classList.contains('previous-year') ||
            li.classList.contains('next-day') ||
            li.classList.contains('next-month')
          ) {
            return;
          }
          ...
          if (!departureDate) {
            ...
            if (!returnDate) {
              if (isDateBefore(departureDate, calendarTitle, e.target.innerText)) {
                document.querySelectorAll('.between-days').forEach((date) => {
                  date.classList.remove('between-days');
                });
                ...
                const days = document.querySelectorAll('.calendar-body li');
                let isBetween = false;
                for (const day of days) {
                  const dayText = parseInt(day.innerText);
                  if (isDateBetween(departureDate, returnDate, { year: parseInt(year), month: parseInt(monthName), day: dayText })) {
                    day.classList.add('between-days');
                    if (dayText === returnDate.day) {
                      isBetween = false;
                    } else {
                      isBetween = true;
                    }
                ...
                modalCarendarReciver(departureDate, returnDate);
              }
            } else {
              document.querySelectorAll('.go-day, .come-day, .between-days').forEach((date) => {
                date.classList.remove('go-day', 'come-day', 'between-days');
              });
              li.classList.add('go-day');
              departureDate = {
                year: parseInt(calendarTitle.split(' ')[0]),
      ...
      function isDateBetween(departureDate, returnDate, targetDate) {
        const targetTimestamp = new Date(targetDate.year, targetDate.month - 1, targetDate.day).getTime();
        const departureTimestamp = new Date(departureDate.year, departureDate.month - 1, departureDate.day).getTime();
        const returnTimestamp = new Date(returnDate.year, returnDate.month - 1, returnDate.day).getTime();
      
        return targetTimestamp > departureTimestamp && targetTimestamp < returnTimestamp;
      }
